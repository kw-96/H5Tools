 ...
 */
export async function findAndCloneButtonImageFromGameInfo(nineGridFrame: FrameNode): Promise<RectangleNode | null> {
  try {
    // 在自适应模块中查找游戏信息容器
 ...
 */
export function getDownloadButtonTextStyle(nineGridFrame: FrameNode): { fontName: FontName, fills: Paint[] } | null {
  try {
    const adaptiveModule = nineGridFrame.parent;
    if (!adaptiveModule || adaptiveModule.type !== 'FRAME') {
      return null;
    ...
    if (textNode) {
      return {
        ...
        fills: JSON.parse(JSON.stringify(textNode.fills)) as Paint[]
      };
    }
    return null;
  } catch (error) {
 ...
 */
export async function createButtonContainer(
  mainContainer: FrameNode,
  nineGridFrame: FrameNode,
  options: { name: string; text: string; x: number; y: number; width: number; height: number; textFontSize?: number }
): Promise<FrameNode> {
  ...
  // 统一用游戏信息中的按钮底图
  const buttonImage = await findAndCloneButtonImageFromGameInfo(nineGridFrame);
  if (buttonImage) {
  ...
  textNode.fontSize = options.textFontSize ?? 50;
