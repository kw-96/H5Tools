...
class CarouselModuleBuilder {
    ...
    setupFrameLayout() {
        figma_utils_1.NodeUtils.setupAutoLayout(this.frame, 'VERTICAL', 30, 90);
    }
    // 添加标题容器 - 与活动内容模块保持一致
    async addTitleContainer() {
        ...
        // 创建标题容器：1080x120px
        const titleContainer = figma_utils_1.NodeUtils.createFrame("标题容器", 1080, this.TITLE_HEIGHT);
        titleContainer.fills = []; // 透明背景
        // 添加标题背景图片 - 1080x120px（与活动内容模块一致）
        if (this.content.titleBgImage) {
            try {
                const titleBgImage = await figma_utils_1.ImageNodeBuilder.insertImage(this.content.titleBgImage, "标题背景图片", 1080, 120);
                if (titleBgImage) {
                    titleBgImage.x = 0;
                    titleBgImage.y = 0;
                    figma_utils_1.NodeUtils.safeAppendChild(titleContainer, titleBgImage, '标题背景图片添加');
        ...
        if (this.content.title) {
            ...
            titleText.fills = [figma_utils_1.ColorUtils.createSolidFill({ r: 1, g: 1, b: 1 })]; // 白色
    ...
    async addCarouselPreview() {
        ...
        // 创建轮播预览容器
        const carouselPreview = figma_utils_1.NodeUtils.createFrame("轮播图预览", 1080, this.CAROUSEL_AREA_HEIGHT);
        carouselPreview.fills = []; // 透明背景
        figma_utils_1.NodeUtils.setupAutoLayout(carouselPreview, 'VERTICAL', 0, 0);
        // 添加轮播区域
        await this.addCarouselArea(carouselPreview);
        // 添加轮播按钮
        await this.addCarouselButtons(carouselPreview);
        figma_utils_1.NodeUtils.safeAppendChild(this.frame, carouselPreview, '轮播预览区域添加');
    }
    // 添加轮播区域 - 包含轮播图背景和轮播图
    async addCarouselArea(parent) {
        console.log('添加轮播区域 - 1080x607px');
        // 创建轮播区域容器
        const carouselArea = figma_utils_1.NodeUtils.createFrame("轮播区域", 1080, this.CAROUSEL_AREA_HEIGHT);
        carouselArea.fills = []; // 透明背景
        // 添加轮播图背景 - 1000x540px，白色，居中
        const carouselBg = figma_utils_1.NodeUtils.createFrame("轮播图背景", this.CAROUSEL_BG_WIDTH, this.CAROUSEL_BG_HEIGHT);
        if (this.content.carouselBgImage) {
            // 使用用户上传的背景图片
            try {
                const bgImage = await figma_utils_1.ImageNodeBuilder.insertImage(this.content.carouselBgImage, "轮播图背景图片", this.CAROUSEL_BG_WIDTH, this.CAROUSEL_BG_HEIGHT);
                if (bgImage) {
                    bgImage.x = 0;
                    bgImage.y = 0;
                    figma_utils_1.NodeUtils.safeAppendChild(carouselBg, bgImage, '轮播图背景图片添加');
                }
            }
            catch (error) {
                console.error('轮播图背景图片创建失败:', error);
                // 失败时使用默认白色背景
                carouselBg.fills = [figma_utils_1.ColorUtils.createSolidFill({ r: 1, g: 1, b: 1 })];
            }
        }
        else {
            // 默认白色背景
        ...
        figma_utils_1.NodeUtils.safeAppendChild(carouselArea, carouselBg, '轮播图背景添加');
        // 添加轮播图 - 960x500px，居中在背景上
        if (this.content.carouselImage) {
            try {
                const carouselImageNode = await figma_utils_1.ImageNodeBuilder.insertImage(this.content.carouselImage, "轮播图", this.CAROUSEL_IMAGE_WIDTH, this.CAROUSEL_IMAGE_HEIGHT);
                if (carouselImageNode) {
                    ...
                    figma_utils_1.NodeUtils.safeAppendChild(carouselArea, carouselImageNode, '轮播图添加');
                }
            }
            catch (error) {
                ...
                placeholder.fills = [figma_utils_1.ColorUtils.createSolidFill({ r: 1, g: 0.3, b: 0.3 })]; // 红色占位
                placeholder.x = (1080 - this.CAROUSEL_IMAGE_WIDTH) / 2;
                placeholder.y = (this.CAROUSEL_AREA_HEIGHT - this.CAROUSEL_IMAGE_HEIGHT) / 2 + 0.5;
                figma_utils_1.NodeUtils.safeAppendChild(carouselArea, placeholder, '轮播图占位添加');
            }
        }
        figma_utils_1.NodeUtils.safeAppendChild(parent, carouselArea, '轮播区域添加');
    }
    // 添加轮播按钮 - 按Figma设计：5个圆形按钮，第一个为长椭圆
    async addCarouselButtons(parent) {
        console.log('添加轮播按钮');
        // 创建轮播按钮容器
        const buttonsContainer = figma_utils_1.NodeUtils.createFrame("轮播按钮", 300, this.BUTTON_HEIGHT);
        buttonsContainer.fills = []; // 透明背景
        figma_utils_1.NodeUtils.setupAutoLayout(buttonsContainer, 'HORIZONTAL', 24, 0); // 水平布局，间距24px
        // 第一个按钮 - 长椭圆形，活跃状态
        const activeButton = figma.createEllipse();
        activeButton.name = "活跃按钮";
        activeButton.resize(60, this.BUTTON_HEIGHT);
        activeButton.fills = [figma_utils_1.ColorUtils.createSolidFill({ r: 1, g: 1, b: 1 })]; // 白色填充
        figma_utils_1.NodeUtils.safeAppendChild(buttonsContainer, activeButton, '活跃按钮添加');
        // 其他4个按钮 - 圆形，非活跃状态
        for (let i = 1; i < 5; i++) {
            ...
            button.fills = []; // 透明填充
            button.strokes = [figma_utils_1.ColorUtils.createSolidFill({ r: 1, g: 1, b: 1 })]; // 白色边框
            button.strokeWeight = 2;
            figma_utils_1.NodeUtils.safeAppendChild(buttonsContainer, button, `按钮${i + 1}添加`);
        }
        // 按钮容器居中
        buttonsContainer.x = (1080 - buttonsContainer.width) / 2;
        figma_utils_1.NodeUtils.safeAppendChild(parent, buttonsContainer, '轮播按钮容器添加');
    }
}
